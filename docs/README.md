...
Use ←/→ to step scenes, space to pause, r to restart. **[See all reels →](/forge_docs/see-it-run.html)** — the AI multi-provider flow, source-aligned Bronze, guided forge UX, day-2 ops, and more.
...
The source-aligned acquisition stack (schema **0.7.3**) lands the **SDP / ADP / CDP** Data Mesh vocabulary alongside the medallion `Bronze / Silver / Gold` layers, **six ingestion engines** (`duckdb`, `dlt`, `meltano`, `airbyte`, `kafka-connect`, `debezium`), public extension API `fluid_build.api` v1.0, and the guided `fluid forge` UX (mode picker, welcome scan, slash commands, preview panel). It is not in the pinned 0.8.0 docs baseline yet. See [Source-Aligned Acquisition](/forge_docs/advanced/source-aligned-acquisition.html), [Product Types](/forge_docs/data-products/product-type.html), and [What's coming](/forge_docs/see-it-run.html) for the full picture.
:::
